        ...
        <div class="tab-pane active" id="general">
            <div class="row">
                <div class='col-md-4 col-sm-6 col-xs-12 '>
                    <?= $form->field($model, 'main_category')->dropDownList(ArrayHelper::map(common\models\MainCategory::find()->orderBy(['sort_order' => SORT_ASC])->all(), 'id', 'main_category'), ['prompt' => 'select']) ?>
                </div>
                <div class='col-md-4 col-sm-6 col-xs-12'>
                    <?php
                    if (!$model->isNewRecord) {
                        ...
                        $categorys = ArrayHelper::map(Category::find()->orderBy(['category' => SORT_ASC])->all(), 'id', 'category');
                    }
                ...
                <?php
                if ($model->isNewRecord) {
                    $ean_no = $this->context->generateProductEan();
                    $model->item_ean = $ean_no;
                }
                ?>
                <div class='col-md-4 col-sm-6 col-xs-12'>
                ...
                <div class='col-md-12 col-sm-12 col-xs-12'>
                    ...
                    $pro_types = ArrayHelper::map(\common\models\ProductType::find()->where(['status' => '1'])->all(), 'id', 'product_type');
                    echo $form->field($model, 'type')
                            ->dropDownList($pro_types, [
                                'multiple' => 'multiple',
                                'class' => '',
                                    ]
                    );
                ...
                <div class='col-md-4 col-sm-6 col-xs-12 ' style="display:none">
                    ...
                    if ($model->isNewRecord) {
                        ...
                        $ean_type1 = $model->ean_type == 1 ? 'checked' : '';
                        $ean_type2 = $model->ean_type == 2 ? 'checked' : '';
                    }
                    ?>

                    <input type="radio" class="ean_upc" name="Product[ean_type]" value="1" <?= $ean_type1 ?>> EAN
                    <input type="radio" class="ean_upc" name="Product[ean_type]" value="2" <?= $ean_type2 ?>> UPC
                </div>
                <div class='col-md-4 col-sm-6 col-xs-12 '>
                    <?= $form->field($model, 'ean_value')->textInput(['maxlength' => true]) ?>
                </div>
                <div class='col-md-4 col-sm-6 col-xs-12 '>
                    <?= $form->field($model, 'price')->textInput(['type' => 'number', 'min' => '0', 'autocomplete' => 'off', 'step' => 'any'])->label('Price'); ?>
                </div>
                <div class='col-md-4 col-sm-6 col-xs-12 '>
                ...
                <!--Meta--->
                <div class='col-md-4 col-sm-6 col-xs-12 '>
                    <div class="form-group field-product-search_tag">
                        <label class="control-label" for="product-search_tag">Search Tag </label>
                        <!--<label onclick="jQuery('#modal-7').modal('show', {backdrop: 'fade'});" class="btn btn-icon btn-white tag_cat">Select Tag Category</label>-->
                        <?php
                        if (!$model->isNewRecord) {
                        ...
                        ?>
                        <?= $form->field($model, 'search_tag')->dropDownList(ArrayHelper::map(MasterSearchTag::find()->where(['status' => '1'])->all(), 'id', 'tag_name'), ['class' => 'form-control', 'id' => 'product-search_tag', 'multiple' => 'multiple'])->label(FALSE) ?>
...
<div class="modal fade" id="modal-7">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title7"  field_id="product-search_tag_category">Search Tag Category</h4>
            </div>

            <div class="modal-body">
                <select id="tag_category" class="form-control">
                    <option value="">Select Category</option>
                    <?php
                    $tag_category = common\models\MasterSearchtagCategory::find()->where(['status' => '1'])->all();
                    foreach ($tag_category as $tag_cat) {
                        ?>
                        <option value="<?= $tag_cat->id ?>"><?= $tag_cat->category ?></option>
                    <?php } ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <label class="control-label" for="Brand">Brand Logo(200x110)</label>
                <input name="userImage" type="file" id="brand-image" class="inputFile" /><br/>
                <div class="form-group" style="float: right;">
                    <?= Html::submitButton($model->isNewRecord ? 'Create' : 'Update', ['class' => 'btn btn-success', 'style' => 'margin-top: 18px; height: 36px; width:100px;']) ?>
                </div>
...
//            $('#product-canonical_name').val(canonical);
        ...
        $('#product-product_name').blur(function () {
            var description_text = "Buy " + this.value + " from Al Hajis Perfumes";
